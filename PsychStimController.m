...
if ismember(StimType, [1, 6, 8, 9])     %% drifting or counterphase gratings
...
if ismember(StimType, [1, 2, 6, 8, 9, 11, 12, 13]) %% drifting bars, drifting or counterphase gratings, moving spots
    ...
    switch stim
        ...
        case {1,16,6,8,9}
            ...
            for c = 1:nCond
                ...
                elseif get(handles.StimType,'Value')==9   %%% reverse drift gratings
                    [img cl] = generateCurvGratings_lut(handles.orient(c),handles.freq(c),handles.TempFreq(c),handles.phase(c),handles.contrast(c),Duration, degPerPix,imageRect(3),imageRect(4),FrameHz,black,white,sizeLut,handles.positionX(c),handles.positionY(c),handles.length(c),handles.speed(c));
                end
                % masked stim?
                if get(handles.mask,'Value') ==1
...
if exist('Length0','var')  %% added 20Jul2006
    ...
    set(handles.runningexpt,'Value',runningexpt);
...
% --- Executes on button press in runningexpt.
function runningexpt_Callback(hObject, eventdata, handles)
% hObject    handle to runningexpt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
